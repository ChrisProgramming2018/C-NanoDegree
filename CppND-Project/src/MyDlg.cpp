...
MyDlg::MyDlg() {
  ...
  // connect ( _comboBox, SIGNAL(currentIndexChanged(QString)), pDataPathlLabel, SLOT (setPath()));
  
  // timer for watch
  connect(timer, SIGNAL(timeout()), this, SLOT(showTime()));
  timer->start(100);
  std::cout << "bevor  mydlg" << std::endl;
  std::cout << "end mydlg" << std::endl;
